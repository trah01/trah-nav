...
const CalendarCard = () => {
    ...
    const renderDays = () => {
        ...
        for (let i = firstDayOfMonth - 1; i >= 0; i--) {
            ...
            days.push(
                <div key={`prev-${d}`} className="w-7 h-6 flex items-center justify-center text-[11px] text-slate-300 select-none">
                    {d}
                </div>
            )
        }
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            days.push(
                <div key={`curr-${d}`} className="relative w-7 h-6 flex items-center justify-center group/day" title={hd?.name || ''}>
                    <div className={`w-6 h-6 flex items-center justify-center text-[12px] rounded-full transition-all duration-200 ${isToday ? 'bg-blue-500 text-white font-bold shadow-sm' :
        ...
        for (let i = 1; i <= remainingCells; i++) {
            days.push(
                <div key={`next-${i}`} className="w-7 h-6 flex items-center justify-center text-[11px] text-slate-300 select-none">
                    {i}
                </div>
            )
        }
    ...
    return (
        <div className="h-full glass-card p-1.5 rounded-[24px] shadow-soft flex flex-col items-center justify-between overflow-hidden" onWheel={handleWheel}>
            <div className="flex justify-between items-center w-full px-1.5 mb-1">
            ...
            <div className="flex-1 w-full flex flex-col justify-center">
                <div className="grid grid-cols-7 gap-y-0 text-center">
                    {['日', '一', '二', '三', '四', '五', '六'].map(d => (
                        <div key={d} className="w-7 h-4.5 text-[10px] text-slate-400 font-bold flex items-center justify-center uppercase tracking-tighter">{d}</div>
                    ))}
                    {renderDays()}
                </div>
            </div>
