 ...
 * 搜索引擎设置组件
 * 
 * 根据 Chrome Web Store 政策要求：
 * - 默认使用 Chrome 用户设置的默认搜索引擎
 * - 用户可以选择启用自定义搜索引擎
 */
const SearchEngineSettings = ({ searchEngines, defaultEngine, useCustomSearch = false, onChange }) => {
    ...
    const toggleCustomSearch = () => {
        onChange({ useCustomSearch: !useCustomSearch })
    }
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
                <div className="flex items-center justify-between">
                    <div className="flex-1">
                        <h3 className="text-base font-bold text-slate-900">{t('search.useCustomEngine')}</h3>
                        <p className="text-sm text-slate-500 mt-1">
                            {useCustomSearch
                                ? t('search.usingCustom')
                                : t('search.usingChrome')
                            }
                        </p>
                    </div>
                    <button
                        ...
                        title={useCustomSearch ? t('search.switchToChrome') : t('search.enableCustom')}
                    >
                        {useCustomSearch
                            ? <RiToggleFill size={44} className="text-blue-500" />
                            : <RiToggleLine size={44} className="text-slate-300" />
                        }
                    </button>
                </div>
            </div>
                ...
                <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100">
                    <div className="flex gap-3">
                        ...
                        <div className="text-sm text-blue-800 leading-relaxed">
                            <p className="font-bold mb-1">{t('search.followChrome')}</p>
                            <p>{t('search.followChromeDesc')}</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            {useCustomSearch && (
                <>
                    <div className="flex items-center justify-between">
                        <h3 className="text-base font-bold text-slate-950">{t('search.customEngines')}</h3>
                        <div className="flex gap-3">
                    ...
                    <p className="text-sm text-slate-600 leading-relaxed">
                        {t('search.urlHint').split('{query}')[0]}<code className="bg-slate-100 px-1.5 py-0.5 rounded text-slate-900 font-bold">{'{query}'}</code>{t('search.urlHint').split('{query}')[1]}
                    </p>
                </>
            )}
                                    ...
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <button
                                                ...
                                                title={isDefault ? t('search.currentDefault') : t('search.setDefault')}
                                            >
